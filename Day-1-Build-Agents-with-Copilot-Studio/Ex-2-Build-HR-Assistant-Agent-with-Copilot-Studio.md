...
8. **Copilotチャット**で、**3つのファイル**が正常にアップロードされたら**送信**ボタンをクリックします。
   ...
   注：ドキュメントをアップロードする前にメッセージの入力を求められた場合は、Copilotに「+++Profiles+++」のようなメッセージを入力してください。
   
   ![](Japanese-media/ex2-006.png)

10. Copilot ウィンドウで、もう一度 **副操縦士 チャット** **(1)** をクリックし、**コンテンツの追加する** **(2)** アイコンをクリックして、**このデバイスからアップロード** を選択します。

   ![](Japanese-media/ex2-007.png)

11. ファイルエクスプローラのポップアップで、`C:\LabFiles\Day-1\Data\CV` **(1)** フォルダーに移動し、**最後の2つ (2)** ファイルを選択して **開く (3)** をクリックします。
...
13. チャットボックスに以下のプロンプトを入力し、**送信**ボタンをクリックします:
...
14. 以下のプロンプトを続けて入力し、**送信**ボタンをクリックします。
